...
with st.sidebar:
    ...
    use_custom = st.checkbox("Use custom date range", value=False)
    if use_custom:
        custom_end = st.date_input("End Date", value=datetime.now().date())
        num_weeks = st.number_input("Number of weeks", value=5, min_value=2, max_value=12, step=1)
    else:
        custom_end = None
        num_weeks = 5
    ...
    mqls_achieved = st.number_input("MQLs Achieved", value=0, step=1)
...
<div class="dashboard-header">
    ...
    <p>📅 Week: {current_week[2]} &nbsp;|&nbsp; contify.com &nbsp;|&nbsp; Last {num_weeks} weeks analysis</p>
</div>
""", unsafe_allow_html=True)
...
def generate_pdf(report):
    ...
    pdf.set_xy(15, 15)
    pdf.cell(0, 10, 'Weekly SEO Review — Contify', ln=True)
    pdf.set_font('Helvetica', '', 10)
    pdf.set_xy(15, 27)
    pdf.cell(0, 8, f"Week: {report.get('week_label', '')}  |  Generated: {datetime.now().strftime('%B %d, %Y')}", ln=True)
    pdf.ln(15)
    ...
    for row in report.get('kpi_rows', []):
        pdf.cell(col_widths[0], 7, row[0], 1, 0, 'L')
        pdf.cell(col_widths[1], 7, row[1], 1, 0, 'R')
        pdf.cell(col_widths[2], 7, row[2], 1, 0, 'R')
        pdf.cell(col_widths[3], 7, row[3], 1, 0, 'R')
        pdf.ln()
    ...
    for section in report.get('sections', []):
        ...
        if section.get('table'):
            ...
            for c in cols:
                pdf.cell(cw, 7, str(c)[:20], 1, 0, 'C', True)
            pdf.ln()
            pdf.set_text_color(30, 41, 59)
            pdf.set_font('Helvetica', '', 8)
            for row in data[:15]:
                for val in row:
                    pdf.cell(cw, 6, str(val)[:20], 1, 0, 'C')
                pdf.ln()
...
# Sessions from channel sum (so channel breakdown matches total), Users/Pageviews from traffic query (to avoid double-counting users across channels)
_ch_sessions = ch_weekly.groupby(['Week_Idx','Week_Label']).agg({'Sessions':'sum'}).reset_index()
_t_users = t.groupby(['Week_Idx','Week_Label']).agg({'Users':'sum','Active Users':'sum','New Users':'sum','Pageviews':'sum'}).reset_index()
t_weekly = _ch_sessions.merge(_t_users, on=['Week_Idx','Week_Label'], how='left').fillna(0).sort_values('Week_Idx')
...
def _hero_card(label, value, prev_val):
    ...
    return f'''<div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:14px;padding:20px 16px;text-align:center;
        ...
        <div style="font-size:0.82rem;font-weight:600;color:{arrow_color};margin-top:4px;">{arrow} {abs(chg):.1f}% vs last week</div>
    </div>'''
...
st.markdown(f'''<div style="display:flex;gap:16px;margin-bottom:24px;">
    ...
    {_hero_card("MQLs", mqls_achieved, 0)}
...
# =====================================================
# SECTION 8: ORGANIC USERS & NEW USERS
# =====================================================
section_start("Organic Users & New Users", "🌱")

ou_cur = ou_weekly[ou_weekly.Week_Idx==cur_idx]
ou_prev = ou_weekly[ou_weekly.Week_Idx==prev_idx]
ou_cur_users = int(ou_cur['Users'].sum()) if len(ou_cur) else 0
ou_prev_users = int(ou_prev['Users'].sum()) if len(ou_prev) else 0
ou_cur_new = int(ou_cur['New Users'].sum()) if len(ou_cur) else 0
ou_prev_new = int(ou_prev['New Users'].sum()) if len(ou_prev) else 0

c1, c2, c3 = st.columns(3)
with c1:
    chg = pct_change(ou_cur_users, ou_prev_users)
    st.markdown(f'<div class="metric-mini"><div class="label">Organic Total Users</div><div class="val">{fmt(ou_cur_users)}</div><div class="change">{change_html(chg)} vs last week</div></div>', unsafe_allow_html=True)
with c2:
    chg2 = pct_change(ou_cur_new, ou_prev_new)
    st.markdown(f'<div class="metric-mini"><div class="label">Organic New Users</div><div class="val">{fmt(ou_cur_new)}</div><div class="change">{change_html(chg2)} vs last week</div></div>', unsafe_allow_html=True)
with c3:
    ou_cur_active = int(ou_cur['Active Users'].sum()) if len(ou_cur) else 0
    ou_prev_active = int(ou_prev['Active Users'].sum()) if len(ou_prev) else 0
    chg3 = pct_change(ou_cur_active, ou_prev_active)
    st.markdown(f'<div class="metric-mini"><div class="label">Organic Active Users</div><div class="val">{fmt(ou_cur_active)}</div><div class="change">{change_html(chg3)} vs last week</div></div>', unsafe_allow_html=True)

ou_target_ach = (ou_cur_users / targets['organic_users'] * 100) if targets['organic_users'] > 0 else 0
insight_box([
    f'This week we achieved <strong>{ou_target_ach:.0f}%</strong> of organic users target ({fmt(ou_cur_users)} / {fmt(targets["organic_users"])})',
    f'Organic new users: {change_html(chg2)} ({fmt(ou_cur_new)} vs {fmt(ou_prev_new)})',
], section_key="organic_users")
section_end()
...
for _, r in pivot.iterrows():
    current_tag = ' <span class="week-current">CURRENT</span>' if r['Week_Idx'] == cur_idx else ''
    rows_html += f"<tr><td><strong>{r['Week_Label']}</strong>{current_tag}</td><td>{fmt(r['US'])}</td><td>{fmt(r['UK'])}</td><td>{fmt(r['Europe'])}</td><td>{fmt(r['Other'])}</td></tr>"
